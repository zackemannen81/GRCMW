...
if(adapters[adapter].ctype == C_TYPE_XONE_PAD && data[0] == 0x05 && data[1] == 0x20 && data[2] == 0x07 && data[3] == 0x01 && data[4]  )
{

adapters[adapter].forward_out_reports = 0;
ginfo("Guide button quickfix?\n");
//0x05 0x20 0x07 0x01 0x05
}
if(adapters[adapter].ctype == C_TYPE_XONE_PAD && data[0] == 0x06 && data[1] == 0x20  )
  {
//EDITED BY MRWHITE added check one.!= 0x02
...
//EDITED BY MR WHITE added check two replace 0x02 with 0x06 make controller immune to crash.
//if(adapters[adapter].ctype == C_TYPE_XONE_PAD && data[0] == 0x06 && data[1] == 0x20 && data[3] == 0x02 ) data[3]=0x06; 
      ...
      if(adapter->serial.device != NULL)
      {
        ...
        switch(adapter->ctype)
        {
          case C_TYPE_XONE_PAD:
   	    ...
   	    {
	    ginfo("Authentication successful!");
       	    }
            break;
          default:
            break;
